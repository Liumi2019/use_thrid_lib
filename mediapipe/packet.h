...
class Packet {
 ...
 private:
  ...
  Timestamp timestamp_ = Timestamp_Unset;
};
...
class Holder : public HolderBase {
 public:
  ...
  const std::string DebugTypeName() const final {
    return "MediaPipeTypeStringOrDemangled";
  }
  const std::string RegisteredTypeName() const final {
    const std::string type_string = std::string("MediaPipeTypeString<T>");
    return type_string;
...
inline Packet::Packet(const Packet& packet)
  ...
  std::cout << "Using copy constructor of " << packet.DebugString() << std::endl;
}

inline Packet& Packet::operator=(const Packet& packet) {
  std::cout << "Using copy assignment operator of " << packet.DebugString() << std::endl;
  if (this != &packet) {
...
inline absl::StatusOr<std::unique_ptr<T>> Packet::Consume() {
  ...
  if(ValidateAsType<T>() != absl::OkStatus()) {
  ...
  if (!holder_->HasForeignOwner() && holder_.unique()) {
    std::cout << "Consuming the data of " << DebugString() << std::endl;
    absl::StatusOr<std::unique_ptr<T>> release_result =
        holder_->As<T>()->Release();
    if (release_result.ok()) {
      std::cout << "Setting " << DebugString() << " to empty." << std::endl;
      holder_.reset();
    }
...
inline absl::StatusOr<std::unique_ptr<T>> Packet::ConsumeOrCopy(
  ...
  std::cout << "Copying the data of " << DebugString() << std::endl;
...
inline Packet::Packet(Packet&& packet) {
  std::cout << "Using move constructor of " << packet.DebugString() << std::endl;
  holder_ = std::move(packet.holder_);
  timestamp_ = packet.timestamp_;
  packet.timestamp_ = Timestamp_Unset;
}

inline Packet& Packet::operator=(Packet&& packet) {
  std::cout << "Using move assignment operator of " << packet.DebugString() << std::endl;
  if (this != &packet) {
    holder_ = std::move(packet.holder_);
    timestamp_ = packet.timestamp_;
    packet.timestamp_ = Timestamp_Unset;
...
inline const T& Packet::Get() const {
  ...
  if (holder == nullptr) {
    ...
    std::cout << "Packet::Get() failed: " << status.message() << std::endl;
  }
  return holder->data();
}

inline Timestamp Packet::getTimestamp() const { return timestamp_; }
